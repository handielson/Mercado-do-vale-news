...
Atualizado em 2026-09-24. Objetivo aprovado: eliminar a dependência operacional do Bling, começando pelo fiscal com integração direta à SEFAZ. Evoluir por etapas e testar o ciclo de venda inteiro. Este arquivo é a fonte única do progresso; os documentos vinculados guardam detalhes e evidências.
...
- [x] D06b.9 — Corrigir a orientação da prévia fiscal do Bling após HTTP 422 por excesso de notas. A tela começa por um dia, oferece reduzir o período excedido e avançar após cada importação, mantendo prévia e confirmação por lote (E55–E57). Em produção, a prévia de 23/09/2026 retornou 2 NF-e autorizadas (R$ 224,01), a reimportação controlada confirmou 2 documentos e a leitura posterior mostrou as mesmas duas notas, sem duplicatas. Um dia com mais de 120 documentos continua protegido pelo limite e exige subdivisão por horário antes de ser importado (D06b.11).
- [x] D06b.10 — Colocar o Espaço do Contador como terceira aba no topo de Dados da Empresa, com Faturamento e notas em primeiro plano e gestão de acesso/importação separada (E56). Testes fiscais 36/36, portal 34/34, build e publicação v1.2.464 aprovados. Validação autenticada em produção confirmou totais, conciliação, notas por canal e seção separada de acesso/importação.
- [ ] D06b.11 — Permitir subdivisão segura por horário quando um único dia exceder 120 documentos fiscais no Bling. Confirmar primeiro o filtro horário suportado pela API oficial; preservar prévia, fingerprint e importação atômica de cada subperíodo. Nenhum dia acima desse limite foi importado; implementar e testar antes de processar um dia assim (E57).
...
- [ ] F11a — Cancelar automaticamente NF-e de pedido Shopee/TikTok efetivamente cancelado antes da circulação. Regra do operador em 24/09/2026: `CANCELLED` atual basta para iniciar a avaliação, sem exigir autoria do comprador; agir assim que detectar; 23 horas são margem interna contada da **autorização da NF-e**, não espera. E58–E62: consulta SEFAZ extrai protocolo/hora; conferência consulta pedido atual; evento 110111 é gerado/assinado; monitor recebe webhooks e varre documentos importados, guarda uma tentativa por NF-e, reconcilia o retorno na SEFAZ e alerta pedido em aberto com nota autorizada, sem cancelá-lo. Sinal positivo de envio bloqueia a transmissão; status intermediário permanece pendente de nova consulta. A tela e as rotas de leitura foram publicadas em `v1.2.465`; a correção de status e o ensaio MySQL local entram em `v1.2.466`. **Pendente:** aplicar `025_fiscal_cancellation_monitor.sql` após backup, testar evento real com NF-e própria em homologação, validar com contador os sinais de circulação em cada canal, garantir ingestão contínua de notas novas sem depender de importação manual do Bling, ativar `MDV_FISCAL_AUTO_CANCEL_ENABLED=1` e `MDV_FISCAL_AUTO_CANCEL_HOMOLOGATED=1` após esses testes, publicar e validar produção controlada. Até lá, automação permanece desligada. NF-e 000699 permanece em D06b.4c, sem cancelamento automático retroativo. Não marcar F11a concluído por testes sintéticos.
- [x] F11a.1 — Ensaio local da migration 025 em MySQL descartável, incluindo reaplicação, unicidade por NF-e, vínculo ao documento e backup/restauração: E62. **Não equivale à aplicação na VPS nem ativa a automação.**
