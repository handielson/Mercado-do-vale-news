...
- [x] F01n — Separar visualmente dados gerais/operacionais de fiscal/certificado sem criar uma segunda fonte para a empresa principal. `company_settings` permanece canônico para os módulos antigos; empresas adicionais usam o perfil fiscal próprio. E29: 10 campos compartilhados coerentes em produção, API protegida contra sobrescrita fiscal, 22/22 testes e navegação autenticada aprovada. F01l permanece aberto para conferir com o Bling e preencher Suframa, atividade principal, IE substituta quando aplicável, município IBGE e início da vigência.
...
- [ ] F01p — Contratar/habilitar a API oficial Consulta CNPJ do SERPRO, cadastrar Consumer Key/Secret somente no cofre do servidor e substituir os espelhos públicos por consulta direta em tempo real à base da Receita. Até isso ocorrer, a interface deve mostrar `Consulta direta oficial: Não`.
