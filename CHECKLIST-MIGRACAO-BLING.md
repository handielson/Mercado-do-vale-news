...
- [ ] D06b — Espaço do Contador separado do painel administrativo, com concessão por conta e empresa, formulário D06, auditoria e visão de faturamento operacional/fiscal. Migration 022 e site/API publicados em E39; rotas protegidas e tabelas conferidas. A conta fictícia de ensaio recebeu acesso e abriu `/contador` após o login (release `v1.2.446-accountant-login`). Importador corrigido e publicado em E40/E41. A tela carregou após E42, mas a conferência de unidades detectou que a versão `v1.2.449` multiplicava por 100 valores já armazenados em centavos; correção `v1.2.450` em publicação. Só validar os totais após repetir o teste. Depois importar o histórico Bling em períodos pequenos, conferir isolamento/persistência com o contador real e obter a validação D06.
