...
async function resolveModelCompanyId(companyId) {
  ...
  const rows = await vpsDbSelect('companies', `select=id&slug=eq.${encodeURIComponent(MODEL_COMPANY_SLUG)}&limit=1`);
  const row = Array.isArray(rows) ? rows[0] : null;
  if (!row?.id) throw new Error('Default company not found for model operations');
  return row.id;
}

async function selectModelsFromVpsDb({ companyId, brandId } = {}) {
...
fastify.patch('/products/stock', { preHandler: requireSyncKey }, async (req, reply) => {
  ...
  let matchedRows = [];
  if (sku) {
    [matchedRows] = await pool.query('SELECT id, stock_quantity FROM products WHERE sku=?', [sku]);
    ...
    [matchedRows] = await pool.query('SELECT id, stock_quantity FROM products WHERE bling_id=?', [String(bling_id)]);
    [result] = await pool.query(
...
async function runMigrations() {
  ...
  await addColumnIfMissing('company_settings', 'tiktok_shop_cipher', 'TEXT DEFAULT NULL');
  ...
  await addColumnIfMissing('company_settings', 'tiktok_access_token_expires_at', 'TEXT DEFAULT NULL');
  await addColumnIfMissing('company_settings', 'tiktok_refresh_token_expires_at', 'TEXT DEFAULT NULL');
  await addColumnIfMissing('company_settings', 'tiktok_open_id', 'TEXT DEFAULT NULL');
  ...
  await pool.query(
    `INSERT IGNORE INTO custom_fields
     ...
     VALUES
      ...
      (UUID(), ?, 'version', 'Versao', 'spec', 'text', JSON_ARRAY(), JSON_OBJECT(), NULL, 1, 14),
    ...
    Array(17).fill(defaultCompanyId)
  );
