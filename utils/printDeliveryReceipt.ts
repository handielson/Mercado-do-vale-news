...
export function printDeliveryReceipt(order: OrderWithItems, settings: CompanySettings) {
    ...
    const itemsHTML = order.items.map(item => {
        ...
        const coveredTag = isWarranty && coveredProduct
            ...
            : '';
        return `<tr>
            <td style="padding:6px 8px;border-bottom:1px solid #eee;">${item.product_name}${colorTag}${skuTag}${coveredTag} — ${item.quantity}x ${fmt(item.unit_price)}</td>
