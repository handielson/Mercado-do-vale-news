...
Verificação local da primeira correção: `npm.cmd run test:accountant-portal` 19/19; `node --check` dos servidores, build e `git diff --check` aprovados. A versão `v1.2.449-accountant-revenue-schema` foi publicada e a tela carregou em produção sem erro HTTP. Durante a revisão dos totais, porém, o contrato monetário do sistema confirmou que `sales.total` e `orders.total` são inteiros em centavos, apesar do tipo MySQL `DECIMAL`. A conversão por 100 da versão 1.2.449 inflou os valores operacionais; portanto, aquela tela não foi aceita como validação contábil. A versão `v1.2.450-accountant-revenue-cents` remove essa conversão e mantém os valores em centavos. Nenhuma nota foi importada nem dado fiscal foi gravado. Repetir validação autenticada da tela após publicar 1.2.450; depois testar importação em períodos pequenos e validar com o contador real. D06b permanece aberto.
