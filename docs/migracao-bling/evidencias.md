...
**Estado:** código local preparado, chave de ativação desligada, migration não aplicada, sem evento real transmitido e sem publicação. Pendente homologação fiscal com nota de teste, validação contábil, ingestão automática de notas novas e publicação controlada. O monitor só cobre NF-e já presente em `company_fiscal_documents`; portanto não dá cobertura total a notas que ainda existam apenas no Bling. A NF-e 000699 segue em D06b.4c e não entra em cancelamento retroativo.
