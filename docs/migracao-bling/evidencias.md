...
Testes locais: suíte `test:accountant-portal` 22/22, incluindo número do pedido na normalização, correspondência exata e não vinculação quando o identificador diverge por caixa; verificações sintáticas, build Vite e `git diff --check` aprovados. Pendente publicar API/site, repetir o lote de 22/09/2026 e validar canais e totais na sessão real do contador.
