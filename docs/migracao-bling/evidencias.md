...
Teste local: `npm.cmd run test:accountant-portal` 23/23 cobre diferença de R$ 2,00, nota com pedido operacional pendente, nota com pedido cancelado e ausência de reclassificação automática de venda sem nota. `npm.cmd run test:company-fiscal` 34/34, build e verificações sintáticas aprovados. A rota de empresa fiscal sem vínculo operacional também devolve listas vazias compatíveis com a tela. Publicação e conferência autenticada da versão `v1.2.455-accountant-reconciliation-review` registradas após o deploy. Permanecem pendentes a investigação dos valores com o contador real, o teste de isolamento em uma segunda empresa/conta e a aprovação de D06.
