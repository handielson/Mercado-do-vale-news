...
## E51 — Componentes dos dois valores divergentes (23/09/2026)

Item: D06b.4, aberto. Consulta somente de leitura às rotas existentes de detalhes da NF-e no Bling e do pedido/repasse na Shopee. Foram extraídos apenas número do documento, referência do pedido, situação e componentes monetários necessários; não foram salvos XML, dados de comprador ou chaves fiscais no repositório.

- NF-e 000693 / Shopee `260922N18QBRPP`: o Bling devolve R$ 12,00 de produto e R$ 12,38 de frete, total R$ 24,38. A Shopee devolve R$ 26,31 de total ao comprador, composto aritmeticamente por produto R$ 12,00 + frete pago pelo comprador R$ 12,38 + taxa ao comprador R$ 1,99 - moedas R$ 0,06. Diferença nota menos pedido: -R$ 1,93. O pedido está `SHIPPED` na consulta atual. Os números fecham; a inclusão ou exclusão fiscal da taxa e das moedas depende do contador.
- NF-e 000699 / Shopee `260923S8DHQDU6`: o Bling devolve R$ 24,99 de produto e R$ 32,92 de frete, total R$ 57,91. A Shopee devolve total do pedido R$ 55,91 e desconto de cupom da plataforma de R$ 2,00 em seu detalhe financeiro. A diferença nota menos pedido é +R$ 2,00, numericamente compatível com esse cupom; os campos de repasse deste pedido já refletem cancelamento e não provam isoladamente a composição original do frete ao comprador.

Nova divergência operacional: na consulta de 23/09/2026, a Shopee respondeu `CANCELLED` para o pedido da NF-e 000699, com última atualização informada às 10:09:08 (horário de Pernambuco); a NF-e consta no Bling como situação autorizada, emitida às 10:08:40. A proximidade temporal não prova a sequência exata do cancelamento nem a situação atual perante a SEFAZ. Nenhuma nota, pedido, status ou regra foi alterado. D06b.4a está concluído como conferência aritmética; D06b.4b e D06b.4c exigem conferência documental e decisão do contador. D06b permanece aberto.

## E52 — Consulta de protocolo da NF-e no Espaço do Contador (23/09/2026)

Item: D06b.4d. Foi acrescentada consulta autenticada à SEFAZ-PE em produção por chave da NF-e 55, com o A1 instalado para a empresa selecionada. A rota exige concessão de faturamento do contador, valida que o documento pertence à empresa e que o CNPJ da chave é o do emitente, limita a frequência e devolve código, motivo e horário da consulta. Não grava resultado nem modifica documento fiscal, venda ou conciliação. A interface permite selecionar uma NF-e importada e mostra a resposta para conferência.

Proteção local: testes do cofre fiscal verificam envio mTLS, corpo SOAP e interpretação da resposta; testes do portal cobrem acesso permitido e negado, isolamento da empresa, CNPJ incompatível e ausência de gravação. `test:company-fiscal` 35/35, `test:accountant-portal` 30/30, build, sintaxe e `git diff --check` aprovados. Pendente: conferir a resposta real da NF-e 000699 na SEFAZ em sessão autorizada e submeter o resultado ao contador. A análise fiscal D06b.4b/c e a aprovação D06 continuam abertas.

A primeira validação pública, na versão 460, confirmou site e API saudáveis, mas o teste autenticado da NF-e 000699 retornou `403 Admin required` no proxy do site: o encaminhamento tratava todo POST protegido como administrativo antes de avaliar a concessão do contador. A correção v1.2.461 libera somente a forma exata da rota de consulta para contas autenticadas; a rota interna continua exigindo concessão de faturamento e empresa/chave correspondentes. Outros POSTs continuam sob a regra administrativa. Teste estático cobre os três servidores e verbos/caminhos negados. A resposta da SEFAZ continua pendente de validação pública após esta correção.

## E53 — Validação autenticada da consulta SEFAZ e pendência fiscal (23/09/2026)

Após a publicação v1.2.461 (commit `b5baeccb`), o Espaço do Contador abriu a NF-e 000699 e consultou o serviço de protocolo da SEFAZ-PE em produção com o A1 da empresa. A resposta visível foi `cStat 100 — Autorizado o uso da NF-e`, capturada em 23/09/2026 às 20:59:20 no horário exibido pelo navegador. O pedido Shopee vinculado havia retornado `CANCELLED` na consulta operacional; portanto, existe divergência entre o estado atual do pedido e a autorização fiscal. A consulta não alterou nenhum registro.

Verificações públicas: API `/status` saudável e `mysql.ok=true`; `/VERSION.json` confirmou v1.2.461 e `/var/www/mdv-site/releases/20260923-235656-accountant-sefaz-proxy-auth`; POST do proxy sem autenticação respondeu 401. O painel autenticado devolveu o código e motivo da SEFAZ. D06b.4d está concluído; D06b.4b/c e D06 seguem abertos para decisão documentada do contador sobre valores e providência fiscal. Não presumir que cancelamento do pedido cancele automaticamente a NF-e.

## E54 — Registro de análise por NF-e no Espaço do Contador (23/09/2026)

Item: D06b.4e, em andamento. Ambiente local e MySQL 8.4 descartável. Estado anterior: a consulta SEFAZ e a fila de conciliação mostravam as divergências, mas não havia registro por documento da análise dos valores e da providência proposta pelo contador. A alteração acrescenta formulário de rascunho/conclusão por NF-e, com tratamento monetário, providência proposta, justificativa, fontes e identificação profissional. A API exige concessão de edição à empresa, preserva versão para evitar sobrescrita e audita a gravação. Não atualiza documento fiscal, pedido, faturamento nem calcula tributo.

Procedimento e resultado: `npm.cmd run test:accountant-portal` 33/33, `npm.cmd run test:company-fiscal` 35/35, teste estático do proxy, sintaxe dos servidores e build aprovados. Em MySQL descartável, a migration 024 passou duas vezes; rascunho, rejeição da versão antiga, preservação do estado autorizado da nota e backup/restauração passaram. Não foi criada revisão fictícia em produção. Pendente: publicar API/migration e site, abrir o formulário em sessão autenticada e confirmar que a rota não aceita acesso anônimo. D06b.4b/c só terminam após o contador real registrar a decisão documental.

Publicação e validação: v1.2.462, commit `1d9cc65b`, tag enviada para `main`; a migration 024 foi aplicada e validada no deploy da API, e `mdv-api` reiniciou online. Site ativo em `/var/www/mdv-site/releases/20260924-003516-accountant-note-review`. `/VERSION.json` corresponde à versão/release, API `/status` retornou `mysql.ok=true`, homepage e `/contador` HTTP 200, POST da revisão sem autenticação HTTP 401. Na sessão autenticada, seleção da NF-e 000699 abriu o formulário com o pedido correto, estado Rascunho e campos vazios, sem erro de API. Nenhuma revisão real foi salva; a gravação e concorrência foram verificadas apenas no MySQL descartável. D06b.4e está aprovado como fluxo disponível; D06b.4b/c, D06 e aprovação do contador real permanecem pendentes.

## E55 — Período excessivo na prévia do Bling (23/09/2026)

Item: D06b.9, em andamento. Ambiente: erro HTTP 422 informado pelo operador na rota `preview-bling` em produção; diagnóstico no código e teste local. A UI iniciava no primeiro dia do mês, enquanto a coleta canônica interrompe ao ultrapassar 120 NF-e/NFC-e no período. O 422 protege a conferência e não grava documentos. A correção inicia a seleção no dia local atual, informa o limite real, oferece reduzir um intervalo excedido ao primeiro dia e, após uma importação diária confirmada, avançar para o próximo dia. Cada dia ainda exige prévia e confirmação próprias. Um único dia com mais de 120 documentos continua pendente de subdivisão por horário; o sistema não o importa parcialmente.

Validação local da correção: `test:accountant-portal` 34/34, build e `git diff --check` aprovados. Naquele momento não houve nova consulta ao Bling nem importação real; publicação e teste operacional estão registrados abaixo e em E57.
...
Item: D06b.10, concluído. Estado anterior observado na captura do operador: a área Fiscal e certificado reunia cadastro, matriz contábil, importação e acessos numa página longa; a lista de notas e faturamento existia apenas no portal separado `/contador`, sem entrada clara na tela administrativa. A nova terceira aba superior Espaço do Contador abre primeiro Faturamento e notas, reutilizando o relatório da API já autorizado para administrador. Validação contábil e Acessos e importação são seções distintas; Fiscal e certificado mantém cadastro e A1. Nenhuma nota ou empresa foi alterada.

Validação local: `test:company-fiscal` 36/36, `test:accountant-portal` 34/34, build e `git diff --check` aprovados. Publicação: v1.2.464, commit `8a374449`, tag `v1.2.464-accountant-top-navigation`, release ativo `/var/www/mdv-site/releases/20260924-011349-accountant-top-navigation`. `/VERSION.json` confirmou versão e release, API `/status` manteve `mysql.ok=true`, e as rotas `/admin/settings/company#contador` e `/contador` responderam HTTP 200.

Validação autenticada no painel administrativo em produção: após recarregar a página, a terceira aba superior apareceu. Ao abri-la, o relatório carregou a empresa Mercado do Vale, totais de faturamento, conciliação de pedidos e tabela de sete NF-e identificadas por canal (Shopee e TikTok Shop). A seção Acessos e importação exibiu o contador autorizado e o formulário de prévia iniciado em um único dia. Nenhuma prévia/importação real foi executada nesse teste; D06b.9 segue pendente desse acompanhamento operacional. A página foi deixada na seção Faturamento e notas.

## E57 — Prévia diária e reimportação fiscal controlada (24/09/2026)

Item: D06b.9, concluído para lotes diários dentro do limite. Com a sessão administrativa da empresa principal, a prévia sem gravação de 23/09/2026 retornou 2 NF-e autorizadas, 0 canceladas, 0 NFC-e e total autorizado de R$ 224,01. Após conferir empresa, data, quantidade e total, a importação com fingerprint respondeu 2 documentos conferidos, 2 autorizados e 0 cancelados. Trata-se de reimportação das notas já presentes, para provar a atualização sem duplicação; o Bling foi apenas consultado, sem alteração ou exclusão.

Leitura posterior no Faturamento e notas, filtrada para 23/09/2026, mostrou exatamente NF-e 000699 (Shopee, R$ 57,91) e NF-e 000698 (TikTok Shop, R$ 166,10), total de R$ 224,01. `npm.cmd run test:accountant-portal` repetido: 34/34. A proteção de 120 documentos por conferência continua ativa; um único dia acima desse limite não é importável com o contrato atual e fica pendente de subdivisão horária, registrada em D06b.11. Não houve emissão, cancelamento ou mudança de regra tributária neste teste.

## E58 — Preparação da conferência de cancelamento fiscal (24/09/2026)

Item: F11a, em andamento. Estado anterior: pedido cancelado no canal e NF-e autorizada na SEFAZ podiam divergir; o banco fiscal guarda emissão e chave, mas não protocolo/hora de autorização nem prova de não circulação. A instrução do operador é detectar cancelamento antes do envio e agir imediatamente, usando 23 horas apenas como margem operacional para NF-e. A [SEFAZ-PE](https://www.sefaz.pe.gov.br/Legislacao/Tributaria/Documents/Legislacao/Informativos%20Fiscais/NF-e%20%20-%20NOTA%20FISCAL%20ELETR%C3%94NICA.pdf) exige que a mercadoria não tenha circulado e prevê prazo ordinário de 24 horas; NFC-e segue prazo distinto e está fora deste fluxo. O [Portal NF-e](https://www.nfe.fazenda.gov.br/portal/exibirArquivo.aspx?conteudo=LrBx7WT9PuA%3D) define o evento de cancelamento 110111 com protocolo e justificativa.

Implementação local: `consultInvoice` extrai `nProt` e `dhRecbto` apenas do protocolo de autorização 100 cuja chave corresponde à solicitada. Nova rota administrativa somente de leitura consulta em tempo real pedido Shopee/TikTok e SEFAZ, confere empresa/chave/modelo e retorna motivos de bloqueio. Status histórico `mobile_sale_events`, devolução e pedido de outro ID não autorizam cancelamento. A atribuição do cancelamento ao comprador também precisa estar explícita; sem ela, a justificativa de desistência não é usada. Sem campo positivo confiável de pré-envio, o avaliador retorna `pre_shipment_unverified`; portanto **nenhuma transmissão de evento ou alteração fiscal é executada**. Justificativa candidata: “Desistencia do cliente antes do envio; operacao comercial nao realizada.”, sujeita a validação antes da emissão do evento.

Teste: dados sintéticos; 31 testes de portal, cofre e avaliação aprovados, incluindo status da plataforma cancelado sem prova de pré-envio, chave/CNPJ divergentes, protocolo ausente e janela acima de 23 horas. `node --check` das rotas e do servidor aprovado. Pendente: evidência oficial e observada de pré-envio em cada API, XML assinado e resposta SEFAZ em homologação, persistência/auditoria do evento, idempotência, agendamento/webhooks, validação contábil e publicação. O caso real da NF-e 000699 permanece sob D06b.4c; não se inferiu que ainda esteja no prazo ou apto a cancelamento.

## E59 — XML do evento fiscal e conferência visível (24/09/2026)

Item: F11a, em andamento. Ambiente: implementação e testes locais; nenhuma chamada de cancelamento à SEFAZ real. A página administrativa de faturamento agora oferece “Conferir cancelamento” para NF-e vinculada a Shopee/TikTok. A resposta mostra situação atual do pedido, autoria do cancelamento quando a API informar, estado da NF-e na SEFAZ e bloqueios. A página do contador mantém somente sua consulta fiscal original; a conferência do marketplace fica restrita ao administrador.

Componente fiscal isolado: montagem do `envEvento` 1.00 com `tpEvento=110111`, `Id=ID+tpEvento+chNFe+01`, órgão 26, protocolo de autorização, justificativa e ambiente; assinatura XML do `infEvento` com o A1, validação local da própria assinatura e transporte mTLS ao NFeRecepcaoEvento4 de PE. O retorno só é considerado aceito quando HTTP 200, lote 128, evento 135 ou 155, mesma chave/tipo/sequência/ambiente e protocolo de evento com 15 dígitos. Fontes oficiais: [MOC/NT do evento](https://www.nfe.fazenda.gov.br/portal/exibirArquivo.aspx?conteudo=DEisbx2XCIs%3D), [padrão de assinatura](https://www.nfe.fazenda.gov.br/portal/exibirArquivo.aspx?conteudo=HO2V+vzhFBk%3D) e [endpoints da SEFAZ-PE](https://www.sefaz.pe.gov.br/Servicos/nota-fiscal-eletronica/Paginas/url-web-services-prod-homolog.aspx). Bibliotecas adicionadas: `xml-crypto` 6.3.1 e `@xmldom/xmldom` 0.9.12; auditoria das dependências não apontou vulnerabilidade nesses dois pacotes.

Teste: PFX sintético, assinatura verificada, requisição SOAP simulada, resposta 135 válida, rejeição por outra chave/status e HTTP de erro. `npm.cmd run build` aprovado; testes específicos e `git diff --check` aprovados. **Ainda não operacional:** nenhuma rota ou tarefa chama `transmitCancellationEvent`, porque o pré-envio não está comprovado nas respostas reais dos dois canais e ainda faltam persistência, recuperação e homologação real. A ausência de `cancel_by` na Shopee ou de iniciador de cancelamento no TikTok bloqueia a justificativa “desistência do cliente”.

## E60 — Regra CANCELLED e alerta para pedido aberto (24/09/2026)

Nova decisão do operador: status atual `CANCELLED` na Shopee ou no TikTok permite avaliar o cancelamento fiscal sem exigir que a API atribua a iniciativa ao comprador. A justificativa candidata foi alterada para “Pedido cancelado antes do envio; operacao comercial nao realizada.”, sem atribuição falsa. Pedido atual ainda em aberto, com NF-e autorizada, recebe alerta e jamais dispara o evento. Sinais positivos de envio vindos do canal ou do job de expedição TikTok bloqueiam o envio. Permanecem obrigatórios modelo 55, emitente/chave coincidentes, autorização e protocolo da SEFAZ, janela interna de 23 horas e situação fiscal ainda autorizada. A [orientação da SEFAZ-PE](https://www.sefaz.pe.gov.br/Legislacao/Tributaria/Documents/Legislacao/Informativos%20Fiscais/NF-e%20%20-%20NOTA%20FISCAL%20ELETR%C3%94NICA.pdf) mantém a condição de não circulação e o prazo ordinário de 24 horas.

Implementação local: monitor com estado único por documento (`migrations/025_fiscal_cancellation_monitor.sql`), acionado por webhook Shopee/TikTok e varredura a cada minuto somente quando `MDV_FISCAL_AUTO_CANCEL_ENABLED=1` e `MDV_FISCAL_AUTO_CANCEL_HOMOLOGATED=1`. Em resposta incerta, o evento não é retransmitido sem conciliação; após confirmação `101` da SEFAZ, o documento próprio é marcado cancelado e auditado. O painel administrativo exibe alertas de pedidos abertos e falhas; a reimportação do Bling não reverte documento já cancelado. Testes sintéticos de ambos os canais, alerta, envio único, reconciliação e timeout passaram; testes do portal, cadastro fiscal e build também passaram.

**Estado após publicação:** versão `v1.2.465-fiscal-cancellation-alerts`, commit `6b673eda`, tag enviada para `main`, API e site publicados; release `/var/www/mdv-site/releases/20260924-150424-fiscal-cancellation-alerts`. A sessão administrativa autenticada exibiu essa versão e carregou a aba Espaço do Contador com o aviso de pedidos em aberto e três pedidos/notas para conferência. `/status` respondeu HTTP 200 com MySQL saudável; as novas rotas negaram acesso anônimo com 401. A chave de ativação segue desligada, a migration 025 não foi aplicada e nenhum evento real foi transmitido. Pendente homologação fiscal com nota de teste, validação contábil e ingestão automática de notas novas. O monitor só cobre NF-e já presente em `company_fiscal_documents`; portanto não dá cobertura total a notas que ainda existam apenas no Bling. A NF-e 000699 segue em D06b.4c e não entra em cancelamento retroativo.

## E61 — Status transitório não encerra monitoramento (24/09/2026)

Item: F11a, ainda aberto. Após a publicação de E60, revisão da máquina de estados identificou que um status atual desconhecido ou intermediário do marketplace, sem sinal de envio, era gravado como `blocked`. Isso impedia uma nova conferência caso o mesmo pedido passasse posteriormente a `CANCELLED`. A correção mantém esse caso em `pending` e inclui `pending` tanto na varredura quanto no processamento disparado por webhook. Estados com impeditivo real, como envio registrado, continuam bloqueados; pedido em aberto conhecido continua como alerta. O teste sintético `UNKNOWN_WAITING` seguido de `CANCELLED` confirmou um único envio simulado; outro teste confirmou que a varredura reencontra o registro pendente e o processa após a mudança de estado. Oito testes focados de automação e XML fiscal e 35 testes do portal do contador passaram. Preparado para a release `v1.2.466-fiscal-cancellation-pending`. A automação de produção continua desligada e sem migration 025.

## E62 — Ensaio da migration 025 em MySQL local (24/09/2026)

Item: F11a, ainda aberto. O teste de integração fiscal passou a instalar a migration `025_fiscal_cancellation_monitor.sql` duas vezes em um MySQL 8.4 descartável, junto com as migrations fiscais anteriores. Confirmou uma linha por documento, rejeição de documento inexistente pela chave estrangeira, estado de alerta persistido e preservação integral da tabela no ciclo `mysqldump`/restauração isolada. O comando `npm.cmd run test:company-fiscal:mysql` passou. O primeiro ensaio não iniciou porque o Docker Desktop estava parado; após iniciar o Docker local, o mesmo teste passou. Nenhuma conexão com banco externo ou escrita na VPS ocorreu.

**Pendente:** backup atual e plano de verificação antes da aplicação da migration 025 na VPS; ela continua não aplicada em produção. O ensaio de schema não substitui a homologação do evento 110111, a validação contábil nem a ingestão contínua de NF-e própria. As duas chaves da automação permanecem desligadas.
