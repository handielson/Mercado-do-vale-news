...
Verificação local: `npm.cmd run test:accountant-portal` 19/19; `node --check` dos servidores, build e `git diff --check` aprovados. Publicação `v1.2.449-accountant-revenue-schema`, commit `7b8785ef`, tag correspondente, release de site `/var/www/mdv-site/releases/20260923-190558-accountant-revenue-schema` e API online em 23/09/2026. Validação autenticada em `/contador` → Faturamento carregou a empresa e o período padrão, sem erro HTTP, exibiu os cartões de totais e movimentos operacionais. `/` HTTP 200, `/VERSION.json` confirmou a release e API `/status` HTTP 200 com `mysql.ok=true`. A tela informa totais documentais zerados porque nenhum histórico foi importado neste teste. Não houve gravação nem importação de dados fiscais. Próximo passo: testar a importação do histórico em períodos pequenos e validar com o contador real. D06b permanece aberto.
