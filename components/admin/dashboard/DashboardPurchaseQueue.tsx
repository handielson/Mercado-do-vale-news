...
type View = 'pending' | 'quoted' | 'purchased';
...
export const DashboardPurchaseQueue: React.FC = () => {
  ...
  const counts = React.useMemo(() => ({ pending: items.filter(i => i.status === 'pending').length, quoted: items.filter(i => i.status === 'quoted').length, purchased: items.filter(i => i.status === 'purchased').length }), [items]);
  const visible = items.filter((item) => view === 'pending' ? ['pending', 'not_purchased', 'removed'].includes(item.status) : item.status === view);
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-3 md:grid-cols-3">{(['pending', 'quoted', 'purchased'] as View[]).map(tab => <button key={tab} onClick={() => setView(tab)} className={`rounded-2xl border p-4 text-left ${view === tab ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700'}`}><p className="text-xs font-bold uppercase tracking-wide">{tab === 'pending' ? 'Lista principal' : tab === 'quoted' ? 'Orçamentos' : 'Compras efetuadas'}</p><p className="mt-1 text-2xl font-bold">{counts[tab]}</p></button>)}</div>
