...
export function AccountantFiscalFileActions({ companyId, documentId, saleId, adminMode = false }: { companyId: string; documentId?: string; saleId?: string; adminMode?: boolean }) {
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-2">
    <button type="button" disabled={busy} className={button} onClick={() => void open('pdf', true)}>Visualizar</button>
    <button type="button" disabled={busy} className={button} onClick={() => void open('pdf')}>Baixar PDF</button>
    <button type="button" disabled={busy} className={button} onClick={() => void open('xml')}>Baixar XML</button>
    {adminMode && documentId && <label className={button}>Guardar XML original<input aria-label="Guardar XML original" type="file" accept=".xml,application/xml,text/xml" disabled={busy} className="mt-1 block max-w-48 text-xs" onChange={event => { void archive(event.target.files?.[0]); event.target.value = ''; }} /></label>}
  </div>{busy && <p role="status" className="text-xs text-slate-500">Preparando documento…</p>}{saved && <p role="status" className="text-xs text-emerald-700">XML arquivado. PDF disponível sob demanda.</p>}{error && <p role="alert" className="max-w-lg break-words text-xs text-red-700">{error}</p>}</div>;
}
