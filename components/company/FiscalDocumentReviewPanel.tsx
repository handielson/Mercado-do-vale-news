...
export function FiscalDocumentReviewPanel({ companyId, documentId, model = '55' }: { companyId: string; documentId: string; model?: string }) {
  ...
  return <section className="rounded-2xl border border-amber-200 bg-white p-5 space-y-4" aria-labelledby="document-review-title">
    <div><h2 id="document-review-title" className="font-bold text-slate-900">Análise do contador para esta {model === '65' ? 'NFC-e' : 'NF-e'}</h2><p className="mt-1 text-sm text-slate-600">Registre o tratamento dos valores e a providência recomendada com suas fontes. A revisão não cancela, devolve ou corrige a nota e não altera o faturamento.</p></div>
    {busy && !record && <p role="status" className="text-sm">Carregando revisão...</p>}
    {error && <p role="alert" className="text-sm text-red-700">{error}</p>}
    {message && <p role="status" className="text-sm text-green-700">{message}</p>}
    {record && <>
      <p className="text-sm text-slate-700">{record.document.model === '65' ? 'NFC-e' : 'NF-e'} {record.document.number || 's/n'} · Pedido {record.document.orderReference || 'sem vínculo'} · {record.review.reviewState === 'reviewed' ? 'Revisada' : 'Rascunho'}{record.review.reviewedAt && ` em ${new Date(record.review.reviewedAt).toLocaleString('pt-BR')}`}</p>
