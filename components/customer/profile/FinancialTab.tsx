...
import { AlertCircle, CheckCircle2, ChevronDown, ChevronRight, Clock3, CreditCard, ExternalLink, Loader2, ReceiptText } from 'lucide-react';
...
export const FinancialTab: React.FC<FinancialTabProps> = ({ customer }) => {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .then(([debtRows, paymentRows]) => {
                if (cancelled) return;
                setDebts(debtRows);
                setPayments(paymentRows);
            })
            .catch((err: any) => {
                if (!cancelled) setError(err?.message || 'Nao foi possivel carregar o financeiro do cliente.');
            })
            .finally(() => {
                                        ...
                                        {debtPayments.length === 0 ? (
                                            ...
                                            <div key={payment.id} className="flex items-center justify-between gap-4 border-b border-slate-100 px-4 py-3 last:border-b-0">
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="mt-1 text-sm text-slate-500">{formatDate(payment.data_pagamento)} - {payment.metodo_pagamento || payment.forma_pagamento || 'Metodo nao informado'}</p>
                                                </div>
