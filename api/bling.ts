...
export default async function handler(req: any, res: any) {
    ...
    if (resource === 'products') {
        ...
        try {
            ...
            const [byName, bySku, byGtin] = await Promise.all([
                fetch(`${base}&nome=${encodeURIComponent(search)}`, { headers }),
                fetch(`${base}&codigo=${encodeURIComponent(search)}`, { headers }),
                fetch(`${base}&gtin=${encodeURIComponent(search)}`, { headers }),
            ]);
            const nameData = byName.ok ? await byName.json() : { data: [] };
            const skuData = bySku.ok ? await bySku.json() : { data: [] };
            const gtinData = byGtin.ok ? await byGtin.json() : { data: [] };
            const seen = new Set<number>();
            const merged: any[] = [];
            for (const item of [...(nameData.data || []), ...(skuData.data || []), ...(gtinData.data || [])]) {
