...
for (const serverFile of ['vps_server.js', 'vps_server.cjs']) {
  ...
  assert.match(server, /const paymentStatus = String\(sale\.payment_status \|\| ''\)\.trim\(\)\.toLowerCase\(\);[\s\S]*sale\.status \|\| \(paymentStatus === 'paid' \? 'completed' : paymentStatus\)[\s\S]*\['completed', 'cancelled', 'canceled'\]\.includes\(saleStatus\)/, `${serverFile} must use the persisted payment status when the sales table has no status column`);
