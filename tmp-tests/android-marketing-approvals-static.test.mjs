...
assert.match(activity, /Últimas decisões e resultados/);
assert.match(activity, /Aprovação de criativo não significa anúncio em veiculação/);
assert.match(activity, /Pausada — sem veiculação/);
...
assert.match(model, /fun persistentStatusLabel\(\)/);
assert.match(model, /Criativos aprovados — ainda não publicados/);
assert.match(model, /fun publicationExplanation\(\)/);
...
assert.match(gradle, /versionCode = 62/);
assert.match(gradle, /versionName = "0\.13\.5"/);
