...
assert.match(api, /function metaReviewState\(entity, options = \{\}\)/);
...
assert.match(api, /async function confirmedSecurityReviewsByItem/);
assert.match(api, /result\.confirmedByMetaMonitor !== true/);
assert.match(api, /securityConfirmation\?\.adId === String\(target\.adId\)/);
assert.match(api, /options\.securityReviewConfirmed/);
assert.match(api, /effective === 'CAMPAIGN_PAUSED' \|\| effective === 'ADSET_PAUSED'/);
...
assert.doesNotMatch(panel, /Preparar ativação automática/);
assert.match(panel, /Cada campanha é controlada separadamente/);
