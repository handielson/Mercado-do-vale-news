...
for (const serverPath of serverPaths) {
  ...
  const limitGuardWithAi = source.indexOf('if (!hasActivePurchaseFlow && !(detectedIntent.storeStatusRequest || aiIntentPlan?.storeStatusRequest) && recentReplyCount >= replyLimit)', webhookStart);
  const limitGuardWithoutAi = source.indexOf('if (!hasActivePurchaseFlow && !detectedIntent.storeStatusRequest && recentReplyCount >= replyLimit)', webhookStart);
  const limitGuard = limitGuardWithAi >= 0 ? limitGuardWithAi : limitGuardWithoutAi;
