...
assert.match(displayPage, /DISPLAY_APP_VERSION = 'V1\.21'/, 'display deve mostrar versao visual do aplicativo');
...
assert.match(displayPage, /Notificacoes[\s\S]*Revisar permissao/, 'painel deve mostrar status de notificacoes do Totem');
assert.match(displayPage, /Bateria sem restricao[\s\S]*Revisar economia/, 'painel deve mostrar status de bateria sem restricao');
assert.match(displayPage, /Liberar notificacoes/, 'painel deve ter botao para liberar notificacoes');
assert.match(displayPage, /Liberar bateria/, 'painel deve ter botao para liberar bateria sem restricao');
assert.match(displayPage, /Permissoes do app/, 'painel deve ter atalho para permissoes gerais do app');
assert.match(displayPage, /requestNotificationPermission/, 'display deve chamar ponte Android para notificacoes');
assert.match(displayPage, /requestBatteryOptimizationPermission/, 'display deve chamar ponte Android para bateria');
