...
assert.match(api, /function confirmsConfiguredPaused\(entity\)/);
assert.match(api, /entity\?\.status/);
assert.match(api, /=== 'PAUSED'/);
assert.match(api, /Meta separates the configured status from effective_status/);
assert.match(api, /PENDING_REVIEW or IN_PROCESS/);
assert.match(api, /!confirmsConfiguredPaused\(campaign\)/);
assert.match(api, /!confirmsConfiguredPaused\(adset\)/);
assert.match(api, /!confirmsConfiguredPaused\(ad\)/);
assert.match(api, /Meta did not confirm paused ad .*effective_status/);
assert.match(api, /tagMetaExecutionError\(error, 'criar anúncio pausado'/);
assert.match(api, /instagram_accounts/);
assert.match(api, /assertInstagramActorEligibleForAdAccount/);
assert.match(api, /resolveInstagramActorForPage/);
assert.match(api, /instagram_business_account\{id,username,name\}/);
assert.match(api, /não está habilitado para anunciar nesta conta/);
assert.match(api, /conceda a tarefa Anunciar/);
assert.match(api, /error\.metaTraceId = meta\.fbtrace_id/);
assert.match(api, /isRejectedInstagramActor/);
assert.match(api, /adPayload\(false\)/);
