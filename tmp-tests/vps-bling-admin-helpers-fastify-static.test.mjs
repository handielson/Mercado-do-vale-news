...
for (const file of ['vps_server.js', 'vps_server.cjs']) {
  ...
  assert.match(source, /vpsDbUpsert\('profiles'/, `${file} must upsert profile by id`);
  ...
  assert.match(source, /vpsDbInsert\('brands'/, `${file} must create missing brand`);
  assert.match(source, /vpsDbPatch\('models'[\s\S]*brand_id/, `${file} must update model brand_id`);
  ...
  assert.match(source, /vpsDbPatch\('products'[\s\S]*bling_id: Number\(blingId\)/, `${file} must patch product bling_id`);
