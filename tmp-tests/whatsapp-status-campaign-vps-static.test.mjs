...
for (const source of [server, serverCjs]) {
  ...
  assert.match(source, /MAX_WHATSAPP_STATUS_PRODUCTS_PER_RUN = 300/);
  ...
  assert.match(source, /selected_dates JSON NULL/);
  assert.match(source, /include_price TINYINT\(1\) NOT NULL DEFAULT 1/);
  ...
  assert.match(source, /normalizeWhatsAppStatusSelectedDates/);
  assert.match(source, /selectedDates\.includes\(localClock\.dateKey\)/);
  ...
  assert.match(source, /isWhatsAppStatusFullCategoryCampaign/);
  assert.match(source, /resolveWhatsAppStatusDailyLimit/);
  assert.match(source, /await resolveWhatsAppStatusDailyLimit\(campaign\)/);
  assert.match(source, /WHATSAPP_STATUS_TIME_ZONE = 'America\/Sao_Paulo'/);
  assert.match(source, /getWhatsAppStatusLocalClock/);
  assert.match(source, /getWhatsAppStatusDayIndex/);
  assert.match(source, /buildWhatsAppStatusScheduledTime/);
  assert.match(source, /CONVERT_TZ\(created_at, '\+00:00', '-03:00'\)/);
  assert.match(source, /startWhatsAppStatusAutomationVps/);
  assert.match(source, /setInterval\(\(\) => void runWhatsAppStatusAutomationTickVps\(\), 60_000\)/);
  assert.match(source, /startWhatsAppStatusAutomationVps\(\);/);
  assert.doesNotMatch(source, /DATEDIFF\(CURDATE\(\), start_date\)/);
  assert.doesNotMatch(source, /DATE_ADD\(start_date, INTERVAL repeat_days DAY\)/);
  ...
  assert.match(source, /product\?\.marketing_background_url/);
  ...
  assert.match(source, /groupWhatsAppStatusProductsByVariation\(inStock\)[\s\S]{0,180}filter\(\(product\) => isWhatsAppStatusProductEligible\(product, includePrice\)\)/);
  ...
  assert.doesNotMatch(source, /function getWhatsAppStatusCardPlan/);
  assert.doesNotMatch(source, /A vista no PIX:.*formatWhatsAppStatusMoney/);
  assert.doesNotMatch(source, /Cartao:.*cardPlan/);
  ...
  assert.match(source, /resolveWhatsAppStatusVideoUrls/);
  assert.doesNotMatch(source, /resolveWhatsAppStatusVideoUrl\(product\)/);
  assert.match(source, /buildWhatsAppStatusVideoCandidates/);
  assert.match(source, /function buildWhatsAppStatusVideoCandidates[\s\S]{0,1800}item\?\.marketing_video_url/);
  assert.match(source, /function buildWhatsAppStatusVideoCandidates[\s\S]{0,1800}item\?\.video_url/);
  assert.match(source, /product\?\.status_group_products/);
  assert.match(source, /const byColor = new Map\(\)/);
  assert.match(source, /candidates: Array\.from\(new Set\(\[\.\.\.group\.marketing, \.\.\.group\.fallback\]\)\)/);
  assert.match(source, /videos\.map\(\(video\) => \(\{ type: 'video', url: video\.url, color: video\.color/);
  assert.match(source, /enqueueWhatsAppStatusMediaBatchVps/);
  assert.match(source, /whatsAppStatusMediaQueueVps = Promise\.resolve\(\)/);
  assert.match(source, /convert: true/);
  assert.match(source, /\{ type: 'image', url: image, color: '' \}/);
  assert.match(source, /WAHA_STATUS_MEDIA_INTERVAL_MS/);
  assert.match(source, /WAHA_STATUS_MEDIA_INTERVAL_MS \|\| 8000/);
  ...
  assert.match(source, /WAHA_STATUS_MAX_MEDIA_PER_PRODUCT \|\| 12/);
  assert.match(source, /requestTimeoutSeconds \* maxMediaPerProduct \+ mediaIntervalSeconds \* \(maxMediaPerProduct - 1\) \+ 30/);
...
assert.match(service, /Number\(row\.daily_limit\) === 0/);
assert.match(service, /Math\.min\(300/);
...
assert.match(service, /selected_dates: selectedDates/);
assert.match(service, /include_price: row\.include_price/);
...
assert.doesNotMatch(panel, /calculateInstallmentFromFees|paymentFeesService/);
...
assert.match(marketingStudio, /whatsappSchedulerView === 'status'[\s\S]{0,120}<WhatsAppStatusCampaignPanel \/>/);
assert.match(marketingStudio, /Status automático/);
assert.match(marketingStudio, /Stories avulsos/);
assert.match(marketingStudio, /<SocialStorySchedulerPanel defaultDestinations=\{\['whatsapp'\]\} \/>/);
assert.match(marketingStudio, /<SocialStorySchedulerPanel defaultDestinations=\{\['instagram'\]\} \/>/);
assert.doesNotMatch(marketingStudio, /<SocialStorySchedulerPanel \/>\s*<WhatsAppStatusCampaignPanel \/>/);
...
assert.match(deployServerOnly, /WAHA_STATUS_MEDIA_INTERVAL_MS: process\.env\.WAHA_STATUS_MEDIA_INTERVAL_MS \|\| '8000'/);
