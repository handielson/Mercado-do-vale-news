...
assert.match(resolver, /classifiedStoreLocationV244/);
const executeResolver = new Function('$json', resolver);
const locationResult = executeResolver({ conversation: 'Onde fica a loja?', intencao: 'localizacao_loja' });
assert.equal(locationResult[0].json.conversationAction, 'consultar_localizacao_loja');
const classifiedLocationResult = executeResolver({ conversation: 'Pode me orientar?', intencao: 'localizacao_loja' });
assert.equal(classifiedLocationResult[0].json.conversationAction, 'consultar_localizacao_loja');
