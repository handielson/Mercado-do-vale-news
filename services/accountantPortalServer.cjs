...
function registerAccountantPortalRoutes(app, { pool, getBearerAuthContext, enabled = process.env.MDV_COMPANY_FISCAL_ENABLED === '1', importBlingDocuments }) {
  ...
  app.get('/accountant/companies/:id/revenue', { preHandler: requireCompanyAccess('revenue') }, async req => {
    ...
    const [onlineRows] = await pool.query(
      `SELECT 'online' AS channel,id AS external_sale_id,COALESCE(status,'pending') AS status,total AS total_cents,COALESCE(paid_at,created_at) AS occurred_at,COALESCE(customer_name,'') AS customer_name
         FROM orders WHERE company_id=? AND COALESCE(paid_at,created_at)>=? AND COALESCE(paid_at,created_at)<? ORDER BY COALESCE(paid_at,created_at) DESC LIMIT 10000`,
      [profile.operational_company_id, `${from} 00:00:00`, `${exclusiveDate} 00:00:00`]
    );
    const byKey = new Map();
