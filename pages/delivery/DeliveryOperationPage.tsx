...
import { Bike, Camera, CheckCircle2, Loader2, MapPin, Phone, QrCode, MessageCircle } from 'lucide-react';
...
const DeliveryOperationPage: React.FC = () => {
    ...
    const [proofs, setProofs] = useState<CustomerDeliveryProof[]>([]);
    ...
    const load = async () => {
        ...
        try {
            ...
            setJob(data.job);
            setProofs(Array.isArray(data.proofs) ? data.proofs : data.proof ? [data.proof] : []);
        } catch (error) {
            ...
            <div className="mx-auto max-w-3xl px-4 py-6">
                ...
                <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center justify-between gap-3">
                        <h2 className="flex items-center gap-2 text-base font-semibold text-slate-900"><QrCode className="h-5 w-5" />Pagamento da entrega</h2>
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{job.payment_status}</span>
                    </div>
                    <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">O valor da entrega ja esta incluido e quitado na venda. Nao cobre nem gere Pix adicional do cliente.</p>
                </section>
                ...
                <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    {proofs.length > 0 && (
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                            {proofs.map((proof) => (
                                <a key={proof.id} href={proof.image_url} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                                    <img className="h-48 w-full object-cover" src={proof.image_url} alt="Foto de comprovacao da entrega" />
                                    {proof.description && <p className="px-3 py-2 text-xs text-slate-600">{proof.description}</p>}
                                </a>
                            ))}
                        </div>
                    )}
