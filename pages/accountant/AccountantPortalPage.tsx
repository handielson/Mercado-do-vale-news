...
export function RevenuePanel({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const [filter, setFilter] = useState<RevenueFilter>('fiscal');
  ...
  const load = async (period = { from, to }) => {
    ...
    setExpandedSale(''); setReport(null);
  ...
  const cards = useMemo(() => report ? [
    ...
    ['Notas canceladas', report.documentTotals.cancelledDocumentCents, 'text-red-700'],
  ...
  return <section className="space-y-5">
    ...
    {report && <>
      ...
      {report.coverage.note && <p className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Escopo atual:</strong> {report.coverage.note} As datas de pedidos e notas seguem o mesmo dia usado pelo filtro, sem conversão pelo fuso do navegador. Os totais de notas autorizadas e canceladas vêm dos documentos fiscais importados; a vinculação de cada nota à venda aparece separadamente na conciliação. Este relatório não calcula impostos.</p>}
      {view && <section aria-label="Somatório do faturamento" className="space-y-4 rounded-2xl border border-blue-200 bg-blue-50 p-5"><div><p className="font-semibold text-blue-900">{revenueFilterLabels[filter]} · {reportDate(report.period.from)} a {reportDate(report.period.to)}</p><p className="mt-2 text-3xl font-black text-blue-950">{money(view.totalCents)}</p><p className="mt-1 text-sm text-blue-900">{view.count} {view.fiscal ? 'documento(s) autorizado(s), pela data de emissão. Cancelados excluídos.' : 'venda(s) concluída(s), pela data da venda. Canceladas e pendentes operacionais excluídas.'}</p></div><p className="text-sm text-slate-700">Filtro de acompanhamento, sem cálculo de impostos. Venda sem nota não é automaticamente isenta; a apuração deve ser conferida pelo contador. Os totais de documentos e de vendas são bases distintas e não devem ser somados.</p><div className="max-h-80 overflow-auto rounded-xl border border-blue-100 bg-white"><table className="min-w-full text-sm"><caption className="p-3 text-left font-semibold">Evolução mensal do filtro selecionado</caption><thead className="bg-slate-50 text-left"><tr><th className="px-4 py-2">Mês</th><th className="px-4 py-2 text-right">Quantidade</th><th className="px-4 py-2 text-right">Faturamento</th></tr></thead><tbody>{view.months.map(month => <tr key={month.month} className="border-t border-slate-100"><td className="px-4 py-2">{month.month.slice(5)}/{month.month.slice(0,4)}</td><td className="px-4 py-2 text-right">{month.count}</td><td className="px-4 py-2 text-right">{money(month.totalCents)}</td></tr>)}</tbody></table></div></section>}
      <p className="text-sm font-semibold text-slate-600">Totais gerais do período, para comparação</p>
