...
export const PublicProductPage: React.FC = () => {
    ...
    const [hasValidModelPanel, setHasValidModelPanel] = useState(false);
    ...
    useEffect(() => {
        ...
        const fetchProduct = async () => {
            ...
            try {
                ...
                if (!data || data.error) {
                    try {
                        const skuResults = await vpsApiService.getProducts({ sku: slug, status: 'all', limit: 5, noCache: true });
                        const skuMatch = (skuResults || []).find(p =>
                            String(p.sku || '').trim().toLowerCase() === slug.toLowerCase()
                        );
                        if (skuMatch) {
                            data = skuMatch;
                        }

                        const searchTerms = data && !data.error ? [] : Array.from(new Set([slug, slug.replace(/-/g, ' ')]));
    ...
    const adminProductUrl = isAdmin && product?.id
        ...
        : '';
    const adminModelPanelUrl = isAdmin && hasValidModelPanel && productModelId && productModelId !== '0' && productModelId !== 'null'
