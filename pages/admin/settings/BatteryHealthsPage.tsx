...
export function BatteryHealthsPage() {
    ...
    const handleDelete = async (bh: BatteryHealth) => {
        if (!confirm(`Excluir "${bh.label}"?`)) return;

        try {
            await batteryHealthService.delete(bh.id);
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {batteryHealths.length === 0 ? (
                            ...
                            batteryHealths.map((bh) => (
                                <tr key={bh.id} className="hover:bg-slate-50 transition-colors">
