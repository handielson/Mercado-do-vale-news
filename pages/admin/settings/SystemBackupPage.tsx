...
import {
  ...
  type SystemBackupHistoryRecord,
...
function historyTone(state: SystemBackupHistoryRecord['state']): string {
  if (state === 'success') return 'border-emerald-200 bg-emerald-50 text-emerald-700';
  if (state === 'partial' || state === 'vps_saved') return 'border-amber-200 bg-amber-50 text-amber-700';
...
function historyLabel(record: SystemBackupHistoryRecord): string {
  ...
  if (record.state === 'running') return 'Em andamento';
  return 'Registrado';
}

function formatBytes(value?: number | null): string {
  if (!value || value < 0) return 'Tamanho nao registrado';
      ...
      <section className="bg-white border border-slate-200 rounded-lg shadow-sm p-5">
        ...
        {history.length === 0 ? (
          <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
            Nenhum pacote de backup foi encontrado na VPS ainda.
          </div>
        ) : (
          <div className="mt-4 divide-y divide-slate-100">
            {history.map((record) => {
              const synologyOk = record.synologyMirror?.ok === true;
              const synologyDetail = synologyOk
                ? (record.synologyMirror?.path || 'Espelho registrado no Synology')
                : (record.synologyMirror?.error || 'Sem confirmacao de espelho no Synology');
              return (
                <article key={`${record.name}-${record.vpsPackage || ''}`} className="py-4">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="break-all text-sm font-bold text-slate-900">{record.name}</h3>
                        <span className={`rounded-full border px-2 py-0.5 text-xs font-bold ${historyTone(record.state)}`}>
                          {historyLabel(record)}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">
                        Inicio: {formatDateTime(record.startedAt)} · Fim: {formatDateTime(record.finishedAt)}
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs sm:min-w-[260px]">
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <p className="font-bold uppercase text-slate-400">Tamanho</p>
                        <p className="mt-1 font-semibold text-slate-700">{formatBytes(record.vpsPackageSize)}</p>
                      </div>
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        <p className="font-bold uppercase text-slate-400">Origem</p>
                        <p className="mt-1 font-semibold text-slate-700">{record.trigger === 'scheduled' ? 'Automatico' : record.trigger === 'manual' ? 'Manual' : 'VPS'}</p>
                      </div>
                    </div>
                  </div>
