    ...
    if (action === 'Criar rascunhos') {
      ...
      try {
        ...
        const results = await Promise.allSettled(chosen.map(async (product) => {
          ...
          return tiktokShopService.startDraftJob({ product_id: product.id, category_id: mapping.mapping.tiktok_category_id, category_name: mapping.mapping.tiktok_category_name, warehouse_id: warehouse.id });
        }));
