...
export default function TikTokShopBulkPreparation() {
  ...
  const rows = useMemo(() => products.filter((product) => { const item = diagnostic(product, links[product.id]); const tiktokState = links[product.id]?.status || 'NOT_SENT'; return (!brand || product.brand === brand) && (stock === 'all' || (stock === 'positive' ? Number(product.stock_quantity || 0) > 0 : Number(product.stock_quantity || 0) <= 0)) && (state === 'all' || tiktokState === state) && (state !== 'READY' || item.ok); }), [brand, links, products, state, stock]);
