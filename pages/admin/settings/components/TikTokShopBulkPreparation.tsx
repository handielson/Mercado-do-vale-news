    ...
    if (action === 'Criar rascunhos') {
      ...
      try {
        ...
        const results = await Promise.allSettled(chosen.map(async (product) => {
          ...
          let job = await tiktokShopService.startDraftJob({ product_id: product.id, category_id: mapping.mapping.tiktok_category_id, category_name: mapping.mapping.tiktok_category_name, warehouse_id: warehouse.id });
        ...
        const completed = results.filter((result): result is PromiseFulfilledResult<string> => result.status === 'fulfilled').map((result) => result.value);
      ...
      } catch (error: any) { toast.error(error?.message || 'Nao foi possivel iniciar os rascunhos do lote.'); } finally { setRunning(false); }
      return;
    }
