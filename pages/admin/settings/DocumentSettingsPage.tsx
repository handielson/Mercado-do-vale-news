        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                >
                    {saving ? (
                        <>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="w-full md:w-64 flex-shrink-0 bg-white border border-slate-200 rounded-2xl p-3 shadow-sm sticky top-24">
                    <nav className="flex flex-col gap-1.5 overflow-y-auto max-h-[calc(100vh-150px)] pr-2 custom-scrollbar">
                        {([
                            { id: 'receipt', label: 'Cupom Térmico', icon: <Receipt className="w-4 h-4" /> },
                            { id: 'payment_receipt', label: 'Recibo A4', icon: <FileText className="w-4 h-4" /> },
                            { id: 'debt_clearance', label: 'Quitação', icon: <FileText className="w-4 h-4" /> },
                            { id: 'delivery_receipt', label: 'Comprovante', icon: <Package className="w-4 h-4" /> },
                            { id: 'warranty', label: 'Garantia', icon: <Shield className="w-4 h-4" /> },
                            { id: 'extended_warranty', label: 'G. Estendida', icon: <Shield className="w-4 h-4" /> },
                            { id: 'headers', label: 'Cabeçalhos', icon: <FileText className="w-4 h-4" /> },
                            { id: 'extra_page', label: 'Folha Extra', icon: <QrCode className="w-4 h-4" /> },
                            { id: 'tags_dictionary', label: 'Dicion. de Tags', icon: <Tag className="w-4 h-4" /> },
                        ] as { id: TabType; label: string; icon: React.ReactNode }[]).map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${activeTab === tab.id
