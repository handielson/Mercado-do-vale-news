  ...
  return <section className="space-y-3 rounded-xl border bg-white p-5 shadow-sm">
    ...
    <fieldset disabled={disabled || busy} className="space-y-3 disabled:opacity-60">
      ...
      {mode === 'automatic' && <>
        ...
        {source === 'pexels' && <><label className="flex gap-2 text-xs"><input type="checkbox" checked={reviewed} onChange={e => setReviewed(e.target.checked)} />Revisei as selecionadas: contexto correto, sem marcas, pessoas ou acessórios que confundam o produto.</label>
        <button type="button" disabled={!selected.length || !reviewed} onClick={() => void run(async () => { const result = await marketingScenes.import(selected, true); const failed = result.items.filter(i => i.status === 'failed'); const approved = result.items.filter(i => i.status !== 'failed' && i.approved); if (!approved.length) { setMessage(`${failed.length || selected.length} importações falharam${failed[0]?.message ? `. ${failed[0].message}` : '.'}`); return; } await choose(approved[0], productId); setMessage(failed.length ? `${failed.length} importações falharam. O cenário aprovado foi aplicado à prévia.` : 'Cenário aprovado e aplicado à prévia. Agora você pode baixar a arte.'); setItems((await marketingScenes.library()).items.filter(i => i.active)); setSource('library'); setSelected([]); setReviewed(false); })} className="rounded bg-blue-600 px-3 py-2 text-sm text-white">Aprovar, importar e usar nesta arte</button></>}</>}
